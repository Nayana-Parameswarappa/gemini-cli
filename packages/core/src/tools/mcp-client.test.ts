  ...
  describe('McpClient', () => {
    ...
    it('should register tool with readOnlyHint and add policy rule', async () => {
      ...
      expect(mockPolicyEngine.addRule).toHaveBeenCalledWith({
        toolName: 'mcp_test-server_readOnlyTool',
...
describe('connectToMcpServer with OAuth (flag disabled)', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('GEMINI_CLI_ENABLE_MCP_SDK_OAUTH', '0');
...
describe('connectToMcpServer with OAuth (flag enabled)', () => {
  let mockedClient: ClientLib.Client;
  let workspaceContext: WorkspaceContext;
  let testWorkspace: string;
  let mockAuthProvider: MCPOAuthProvider;
  ...
  beforeEach(() => {
    ...
    vi.spyOn(console, 'warn').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});
    vi.stubEnv('GEMINI_CLI_ENABLE_MCP_SDK_OAUTH', '1');
...
describe('connectToMcpServer - OAuth with transport fallback', () => {
  ...
  it('should handle HTTP 404 → SSE 401 → OAuth → SSE+OAuth succeeds when flag is disabled', async () => {
    vi.stubEnv('GEMINI_CLI_ENABLE_MCP_SDK_OAUTH', '0');
  ...
  it('should fail fast on HTTP 404 when OAuth is enabled and flag is enabled', async () => {
    vi.stubEnv('GEMINI_CLI_ENABLE_MCP_SDK_OAUTH', '1');
